...
class control():
    def __init__(self, config: dict) -> None:
        ...
        self.p = GPIO.PWM(13, 50) #GPIO4をPWM設定、周波数は50Hz
    ...
    def run(self, center_pix, num_human_det):
        ...
        degree = int((self.target-center_pix[0])/self.target*self.range) # +:反時計回り　-:時計回り
        #degree += 90 # 0-180に変換
        dc = 2.5 + (12.0-2.5)/180*(degree+90) #角度をDutyCycleに変換
        #DutyCycle dc%
        self.p.ChangeDutyCycle(dc)
        time.sleep(1)
        print(degree, dc, self.target, center_pix[0])
        return dc
